...
func Test_wrapper(t *testing.T) {
	...
	//		err := NewSaga(steps).Execute(ctx)
	//		testtool.AssertNoError(t, err)
	//		testtool.AssertTrue(t, slices.Equal([]string{"before1", "action1", "after1"}, executed))
	//	})
	//	t.Run("compensation", func(t *testing.T) {
	//		var (
	//			ctx      = context.Background()
	//			expErr   = testtool.ErrExpTest
	//			executed []string
	//		)
	//
	//		steps := []Step{
	...
	//					testtool.AssertError(t, actionErr)
	//					testtool.AssertTrue(t, Errors.Is(actionErr, expErr))
	//					return nil
...
func Test_steps(t *testing.T) {
	t.Run("action", func(t *testing.T) {
		...
		t.Run("success_v1", func(t *testing.T) {
			...
			testtool.AssertNoError(t, err)
			testtool.AssertTrue(t, res.Status == StageResultSuccess)
			t.Log(res)
		})
		t.Run("failed_1", func(t *testing.T) {
			var (
				calls uint8
			)
			steps := []Step{
			...
			res, err := NewSaga(steps).Execute(ctx)
			testtool.AssertError(t, err)
			testtool.AssertTrue(t, res.Status == StageResultFail)
			t.Log(res)
		})
	})
	t.Run("compensation", func(t *testing.T) {
			...
			NewStep("step1").
				...
				WithCompensation(
					NewCompensation(func(ctx context.Context, track Track) error {
						...
						//testtool.AssertTrue(t, Errors.Is(actionErr, expErr))
						if len(executed) < 18 {
							return fmt.Errorf("comp err [%d]: %w", len(executed), expErr)
						}
						return nil
					}).WithRetry(NewBaseRetryOpt(4, 5*time.Nanosecond)),
				).WithCompensationOnFail(),
